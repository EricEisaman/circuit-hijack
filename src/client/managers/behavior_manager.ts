...
export interface NeuroBehaviorHandlers {
  readonly handleBehaviorAction: (action: BehaviorAction) => void;
  readonly setNeuroZone: (zone: 'safe' | 'accAnchor', active: boolean) => void;
}
...
export class BehaviorManager {
  ...
  private static neuroHandlers: NeuroBehaviorHandlers | null = null;
  ...
  public static setNeuroHandlers(handlers: NeuroBehaviorHandlers | null): void {
    this.neuroHandlers = handlers;
  }
  ...
  private static updateBehaviors(): void {
    ...
    this.instances.forEach((instance) => {
      ...
      if (triggerResult && !instance.behaviorActive) {
        ...
        this.notifyNeuroZoneEnter(instance);
        ...
        this.removeEffects(instance);
        this.notifyNeuroZoneExit(instance);
  ...
  private static executeAction(instance: BehaviorInstance, action: BehaviorAction): void {
    ...
    if (action.actionType === 'adjustCredits') {
      ...
      action.actionType === 'neuroPulse' ||
      action.actionType === 'adjustNeuro' ||
      action.actionType === 'setInsulaAccCoupling'
    ) {
      this.neuroHandlers?.handleBehaviorAction(action);
    }
  }

  private static notifyNeuroZoneEnter(instance: BehaviorInstance): void {
    const config = instance.config as ProximityTriggerConfig;
    if (config.triggerKind !== 'proximity' || !config.neuroZone) {
      return;
    }
    this.neuroHandlers?.setNeuroZone(config.neuroZone, true);
  }

  private static notifyNeuroZoneExit(instance: BehaviorInstance): void {
    const config = instance.config as ProximityTriggerConfig;
    if (config.triggerKind !== 'proximity' || !config.neuroZone) {
      return;
    }
    this.neuroHandlers?.setNeuroZone(config.neuroZone, false);
  }
}
