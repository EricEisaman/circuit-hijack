...
export class CharacterLoader {
  ...
  public static loadCharacterModel(
    ...
    const defaultName = CONFIG.CIRCUIT_HIJACK.ENABLED
      ? CONFIG.CIRCUIT_HIJACK.DEFAULT_CHARACTER
      : (ASSETS.CHARACTERS[0]?.name ?? '');
    const characterToLoad =
      character ??
      ASSETS.CHARACTERS.find((c) => c.name === defaultName) ??
      ASSETS.CHARACTERS[0];
    if (!characterToLoad) {
