// ============================================================================
// NEUROCHEMISTRY MANAGER — Circuit Hijack simulation
// ============================================================================
...
import type {
  ...
  NeurochemistryState,
  NeuroMovementModifiers,
  NeuroOutcome,
  NeuroPulseKind,
  NeuroRole
} from '../types/neurochemistry';

export interface NeuroHandlers {
  readonly applyBehaviorAction: (action: BehaviorAction) => void;
}

export class NeurochemistryManager {
  private static scene: BABYLON.Scene | null = null;
  private static config: NeurochemistryConfig = NEUROCHEMISTRY_CONFIG;
  ...
  private static outcome: NeuroOutcome = 'playing';
  ...
  public static initialize(scene: BABYLON.Scene): void {
    this.dispose();
    this.scene = scene;
    this.config = NEUROCHEMISTRY_CONFIG;
    ...
    this.keyboardObserver = scene.onKeyboardObservable.add((kb) => {
      if (kb.type !== BABYLON.KeyboardEventTypes.KEYDOWN && kb.type !== BABYLON.KeyboardEventTypes.KEYUP) {
        return;
      }
      const pressed = kb.type === BABYLON.KeyboardEventTypes.KEYDOWN;
      const key = kb.event.key.toLowerCase();
      if (INPUT_KEYS.LABEL_URGE.some((k) => k === key)) {
    ...
    Notification.create({
      scene,
      message:
        'Model-inspired sim: resist cues, avoid drugs, hold [F] in blue ACC zone to label urges, recover in green safe zone.',
      duration: 7000,
      position: 'bottom',
  ...
  public static getSnapshot(): NeurochemistryState {
    return {
      d1: this.d1,
      d2: this.d2,
      rpe: this.rpe,
  ...
  public static getOutcome(): NeuroOutcome {
    return this.outcome;
  }

  public static getMovementModifiers(): NeuroMovementModifiers {
  ...
  public static setNeuroZone(zone: 'safe' | 'accAnchor', active: boolean): void {
    if (zone === 'safe') {
  ...
  public static handleBehaviorAction(action: BehaviorAction): void {
    if (action.actionType === 'neuroPulse') {
      this.applyPulse(action.pulseKind, action.magnitude);
    } else if (action.actionType === 'adjustNeuro') {
      this.adjustState(action.target, action.amount);
    } else if (action.actionType === 'setInsulaAccCoupling') {
      this.insulaAccCoupling = this.clamp01(
        Math.max(this.insulaAccCoupling, action.amount)
      );
    }
  }

  public static handleItemNeuroRole(role: NeuroRole): void {
    switch (role) {
      case 'drug':
        this.applyDrugUse();
        break;
  ...
  public static adjustState(target: NeuroAdjustTarget, amount: number): void {
    switch (target) {
      case 'd1':
  ...
  private static applyPulse(kind: NeuroPulseKind, magnitude: number): void {
    if (kind === 'drug') {
      this.applyDrugUse();
      return;
    }
  ...
  private static tick(dt: number): void {
    ...
    const couplingFactor =
      1 - this.insulaAccCoupling * reg.hungerGrowthCouplingFactor;
    ...
    if (this.inAccAnchor && this.labelUrgeHeld) {
      ...
      this.accAwareness = this.clamp01(
        this.accAwareness - decay.accAwarenessDecayPerSecond * dt
      );
    }
  ...
  private static checkLose(): void {
    if (this.habitEncoding >= this.config.LOSE_WIN.habitEncodingLose) {
      this.outcome = 'lost';
      this.showOutcome(
        'Habit encoded — compulsive seeking dominates',
        'rgba(140, 20, 40, 0.92)'
      );
    }
  }
