...
export interface NeuroPulseAction {
  readonly actionType: 'neuroPulse';
  readonly pulseKind: 'rpe' | 'craving' | 'drug';
  readonly magnitude: number;
}

export interface AdjustNeuroAction {
  readonly actionType: 'adjustNeuro';
  readonly target: 'd1' | 'd2' | 'drugHunger' | 'habitEncoding';
  readonly amount: number;
}

export interface SetInsulaAccCouplingAction {
  readonly actionType: 'setInsulaAccCoupling';
  readonly amount: number;
}
...
export interface ProximityTriggerConfig {
  ...
  /** Marks proximity volume for neuro zone enter/exit (safe zone, ACC anchor). */
  readonly neuroZone?: 'safe' | 'accAnchor';
}
