...
import { NeurochemistryManager } from '../managers/neurochemistry_manager';
...
export class CharacterController {
  ...
  private calculateAirVelocity(
    ...
    if (this.boostActive) {
      const neuroMods = NeurochemistryManager.getMovementModifiers();
      // Character-specific air speed using active character's properties
      const baseSpeed =
        character.speed.inAir * character.speed.boostMultiplier * neuroMods.speedMultiplier;
  ...
  private calculateGroundVelocity(
    ...
    const baseSpeed =
      (this.boostActive
        ...
        : character.speed.onGround) * neuroMods.speedMultiplier;
    const massAdjustedSpeed = baseSpeed / Math.sqrt(characterMass); // Additional mass adjustment for realistic physics
